...
<main class="min-h-screen flex-grow">
    ...
    <ul class="w-30 flex flex-row justify-center m-9">
        @foreach($notes as $note)
            <li class="my-5 border card-shine-effect rounded-lg drop-shadow-xl text-gray-50 p-12 px-5 w-56 m-5">
                ...
                <form action="{{ route('notes.destroy', $note->id) }}" method="POST" class="mt-3">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-indigo-500 hover:text-red-700">Delete</button>
                </form>
            </li>
        @endforeach
    </ul>
    ...
    <section class="text-white max-w-md mx-auto bg-indigo-500 p-8 rounded-md shadow-md mt-8">
        ...
        <form action="{{ route('notes.notes') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="content" id="content" class="bg-indigo-950 p-2 w-full rounded-xl"></textarea>
            </div>
